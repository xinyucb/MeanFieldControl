...
class PathMKV(nn.Module): 
    ...
    def loss_function(
        ...
        for j in range(0, self.N ):
            ...
            t_= t[:, j, :]
            # print("time t", np.shape(t_),t_)
            DW_t = DW[:, j, :]
            dw = self.DW_mask(DW, j)
            dw = dw.view(dw.shape[0],  -1)
            # print("Xt", np.shape(Xt))

            # print("dw", np.shape(DW_t))
            # print('X0_block', np.shape(X0_block))
            X_path = torch.stack(X_buffer, dim=1)     # (M, j+1, d)
            if "A" in self.control_type:
                A_path = torch.stack(A_buffer, dim=1)
            else:
                A_path = None

            net_input = self.setup.get_control(j, t_, X_path, dw, control_type=self.control_type, A_path=A_path)
            
            # get current reward
            control = net_actor(net_input) 
            # print("control", np.shape(control))
            reward += self.setup.f_running( j, t, X_path, control) * self.dt
            
            
            # Forward SDE dynamics (Euler + jump terms)
            drift = self.setup.mu(j, self.dt, t_, X_path, control)
            violatility = self.setup.sigma(DW_t, t, X_path, control)
            Xt = Xt + drift  + violatility 
        ...
        reward += self.setup.g_terminal(X_path_complete) # get the XT
    ...
    def train(self, plot=False):
        ...
        - Creates local copies of network and loads the states.
        ...
        for it in range(self.params_train["iteration"]):
            ...
            ## Validation
            _,  loss, _ = self.simulation_paths(plot)
            # if plot:
            #     pass
    ...
    def simulation_paths(self, plot=False):
        ...
        if plot:
            print("plotting")
            X1 = self.setup.statistics_of_importance(X_buffer)
            plot_one_distribution(X1, self.setup.K, label1=self.control_type)
    ...
    def save_NN(self, filename = None):
        ...
        save_dir = os.path.join(base_dir, "outputNN")
        ...
        torch.save({'net_actor': net_actor.state_dict()}, save_path)
        print(f"✅ The NN has been saved in {save_path}")
    ...
    def save_loss(self, filename=None):
        ...
        #  Get current working directory (works in both scripts and Jupyter)
        base_dir = os.path.dirname(os.path.abspath(__file__))  # current file path
        save_dir = os.path.join(base_dir, "outputLoss")
        ...
        if hasattr(self, 'training_cost_list') and len(self.training_cost_list) > 0:
            tmp = {"train": self.training_cost_list, "val": self.validation_list}

            for key, val in tmp.items():
                # Convert tensor data to numpy
                loss_data = [t.detach().numpy() for t in val]
                df = pd.DataFrame(loss_data, columns=["loss"])

                # Add timestamp to prevent overwriting
                save_path = os.path.join(save_dir, f"{filename}_{key}_loss.csv")

                # Save to CSV
                df.to_csv(save_path, index=False)
                print(f"✅ Loss data saved to: {save_path}")
        else:
            print("⚠️ No training cost data available to save.")

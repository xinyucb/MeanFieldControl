...
class MKV(nn.Module): 
    ...
    def loss_function(
        ...
        - X: Tensor (M, state_dim), state for all the M particles.
        ...
        X_buffer = [X]   # to store states (X) along path
        ...
        for j in range(0, self.N):
            ...
            if self.control_type == "path_dependent":
                ...
                # Xt
                X0_block = X.unsqueeze(1) #xi -> (B, 1, d): just add a time dimension
                ...
                X0_block = X.unsqueeze(1) # Xt -> (B, 1, d)
                ...
                raise ValueError(f"Unknown control_type: {self.control_type}. Use 'path_dependent' or 'markovian'")

            # get current reward
            reward += self.setup.f_running(t0, X, control)* self.dt
            
            # Forward simulate SDE dynamics (Euler + jump terms)
            drift = self.setup.mu(t0, X, control)
            violatility = self.setup.sigma(t0, X, control)
            X = X + drift * self.dt + violatility * DW0
            X_buffer.append(X)

        # Calculate the terminal cost
        reward += self.setup.g_terminal(X) 

...
class PathMKV(nn.Module): 
    ...
    def loss_function(
        ...
        for j in range(0, self.N ):
            ...
            t_= t[:, j, :]
            # print("time t", np.shape(t_))
            DW_t = DW[:, j, :]
            # print("dw", np.shape(DW_t))
            t_block = t_ #  t0 -> (M, 1, d): duplicate the single column to state_dim
            # print('X0_block', np.shape(X0_block))
            X_path = torch.stack(X_buffer, dim=1)     # (M, j+1, d)


            # put X_buffer as the input, move control to Path_Model
            ...
            if self.control_type == "B":
                ...
                net_input = torch.cat([t_block,  X0_block, dw], dim=1)
            ...
            # drift = self.setup.mu(t_, X_path, control)
            # violatility = self.setup.sigma(t_, X_path, control)
            drift = self.setup.mu(self.dt, t_, X_path, control)
            violatility = self.setup.sigma(DW_t, t_, X_path, control)
            # print("Xt", np.shape(Xt))
            Xt = Xt + drift  + violatility 
            # print("drift", np.shape(drift))
            # print("vol", np.shape(violatility))
            # print("Xt_new", np.shape(Xt))
        ...
        reward += self.setup.g_terminal(Xt) # get the XT
